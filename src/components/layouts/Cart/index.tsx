...
const Cart: React.FC<CartProps> = ({ cart, loading }) => {
    ...
    if (!cartList || cartList.products.length === 0) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <p className="text-lg font-bold">No cart data available</p>
            </div>
        );
    }
						...
						<li key={product.id} className="flex items-center p-4 border border-gray-300 rounded-lg">
							...
							<div className="flex-1">
								...
								<p className="text-gray-500">
									<span className="line-through">${product.total.toFixed(2)}</span> 
									<span className="text-red-600 font-bold"> ${product.discountedTotal.toFixed(2)}</span>
								</p>
