...
const Cart: React.FC<CartProps> = ({ cart, loading }) => {
    ...
    useEffect(() => {
        setCartList(cart); // Initialize cartList with cart prop on mount
    }, [cart]);
        ...
        setCartList((prevCart: Carts | null) => {
            ...
            const updatedProducts = prevCart.products.map((product: ProductCart) => {
                if (product.id === productId) {
                    ...
                    return { ...product, quantity: Math.max(1, newQuantity) }; // Prevents quantity from going below 1
                }
                return product;
            });
        ...
        <div className="flex flex-col items-center min-h-screen p-4">
            ...
            <div className="w-full max-w-4xl">
                ...
                <div className="mt-6 border-t pt-4 border-gray-300">
                    <p className="text-lg font-semibold">Total Products: {cartList.totalProducts}</p>
                    <p className="text-lg font-semibold">Total Quantity: {cartList.totalQuantity}</p>
                    <p className="text-lg font-semibold">Total: ${cartList.total.toFixed(2)}</p>
                    <p className="text-lg font-semibold">Discounted Total: ${cartList.discountedTotal.toFixed(2)}</p>
                </div>
            </div>
        </div>
    );
